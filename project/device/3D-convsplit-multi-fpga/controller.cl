...

{
	...
	for (int i = 0; i < config_size; i++) {
		...
		int num_sub_layers = config[i].num_sub_layers;
		...
		int split_weight_n_offset = 0;
		int split_weight_n = weight_n / num_sub_layers;
		for(int sl = 0; sl < num_sub_layers; sl++) {
			if(sl == num_sub_layers - 1 && (weight_n / num_sub_layers) * num_sub_layers != weight_n)
				split_weight_n = weight_n % split_weight_n;
			...
			memrd_data_config.split_weight_n = split_weight_n;
			memrd_data_config.split_weight_n_offset = split_weight_n_offset;
			...
			memrd_weight_config.split_weight_n = split_weight_n;
			memrd_weight_config.split_weight_n_offset = split_weight_n_offset;
			...
			instruction inst;
			inst.conv_loop_cnt = weight_h * (split_weight_n / VEC_SIZE) * weight_t;
			inst.frac_w = frac_w;
			inst.frac_dout = frac_dout;
			inst.frac_din = frac_din;
			inst.num_weight_plates = weight_h * (split_weight_n/VEC_SIZE) * weight_t;
