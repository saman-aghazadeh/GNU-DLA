...
for id in range(var):
	...
	if id == 0:
		...
		if id < 42:
			contentn_mac_replaced = contentn_pep_1_replaced.replace("$soft_mac$", "/*").replace("$end_soft_mac$", "*/").replace("$mac$", "").replace("$end_mac$", "")
		else:
			contentn_mac_replaced = contentn_pep_1_replaced.replace("$soft_mac$", "").replace("$end_soft_mac$", "").replace("$mac$", "/*").replace("$end_mac$", "*/")
		target.write(contentn_mac_replaced)
		target.write("\n\n")
	else:
		...
		if id < 42:
			contentf_mac_replaced = contentf_pep_1_replaced.replace("$soft_mac$", "/*").replace("$end_soft_mac$", "*/").replace("$mac$", "").replace("$end_mac$", "")
		else:
			contentf_mac_replaced = contentf_pep_1_replaced.replace("$soft_mac$", "").replace("$end_soft_mac$", "").replace("$mac$", "/*").replace("$end_mac$", "*/")
		target.write(contentf_mac_replaced)
		target.write("\n\n")
