			...
			while (brick_idx_y != data_h_with_padding-weight_h+1) {
				...
				//for (short plate = 0; plate < num_plates; plate++) {
					...
					if (feature_idx_y == weight_h-1) {
						feature_idx_y = brick_idx_y;
					} else {
						feature_idx_y++;
					}
				//}
				if ((brick_idx_y == data_h_with_padding-1) && (brick_idx_x+(W_VEC-weight_w+1) >= data_w_with_padding-1 ) && plate == num_plates-1){
					brick_idx_y = 0;
				} else if (brick_idx_x + (W_VEC-weight_w+1) >= data_w_with_padding-1 && plate == num_plates-1) {
					brick_idx_y++;
				}

				if (brick_idx_x + (W_VEC-weight_w+1) >= data_w_with_padding-1 && plate == num_plates - 1) {
					brick_idx_x = 0;
				} else if (plate == num_plates - 1){
