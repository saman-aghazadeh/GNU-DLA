/*
 *
 * All kernels for the first layer
 *
 */
			...
			)
{
	...
	for(unsigned int k=0; k<output_num; k++){
		...
		for(int j=0; j<conv_loop_cnt; j++){
			...
			mac_data = read_channel_intel(data_ch_L1);
			mac_weight = read_channel_intel(weight_ch_L1);
		...
		if((contol&0x02)==0x02)
			//by-pass pooling
			write_channel_intel(bypass_ch_L1, conv_ch_in);
		else // to pooling kernel

			write_channel_intel(conv_ch_L1, conv_ch_in);
			...
			)
{
	...
	for(unsigned int k=0; k<input_num; k++){
		...
		conv_ch_out = read_channel_intel(conv_ch_L1);
		...
		if(row_pool_cnt==(pool_size-1)){
			...
			if(col_pool_cnt==(pool_size-1)){
				...
				write_channel_intel(pool_ch_L1, pool_final);
				...
				)
{
	...
	if(local_z==0){
		if((bypass&0x01)==0x01)
			output = read_channel_intel(bypass_ch_L1);

		else
			output = read_channel_intel(pool_ch_L1);
