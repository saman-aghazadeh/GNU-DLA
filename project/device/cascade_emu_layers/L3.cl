/*
 *
 * All kernels for the third layer
 *
 */
 
__kernel
__attribute__((task))
void memReadDeser_L3(
	// Param Ports,
	uchar  data_dim1,
	uchar  data_dim2,
	ushort data_dim3,
	...
	for (unsigned short dim3 = 0; dim3 < data_dim3; dim3++) {
		for (unsigned char dim2 = 0; dim2 < data_dim2; dim2++) {
			for (unsigned char dim1 = 0; dim1 < data_dim1; dim1++) {
				...
				buf = read_channel_intel (sdeser_ch_L2);
...
/*
                if (dim1 >= 0 && dim1 <= 3) {
                    if (dim2 >= 0 && dim2 <= 3) {
                        if (dim3 >= 0 && dim3 <= 3) {
                            printf ("[deser L3] at x=%d,y=%d,z=%d = [", dim1, dim2, dim3);
				...
				Item:for(unsigned int  win_itm_xyz=0; win_itm_xyz<item_loop_bound; win_itm_xyz++){
							...
							if(gp_num_x*CONV_GP_SIZE_X+gp_item_idx_x<conv_x){
								...
								if(output_idx_dim1==0 && output_idx_dim2==0 && output_idx_dim3==0){
									...
									write_channel_intel(bias_ch_L3, bias_ch_in);
								...
								write_channel_intel(data_ch_L3, data_ch_vec);
								...
								write_channel_intel(weight_ch_L3, weight_ch_vec);
			...
			)
{
	...
	for(unsigned int k=0; k<output_num; k++){
		bias_ch_out = read_channel_intel(bias_ch_L3);
		...
		for(int j=0; j<conv_loop_cnt; j++){
			mac_data = read_channel_intel(data_ch_L3);
			mac_weight = read_channel_intel(weight_ch_L3);
		...
		if((contol&0x02)==0x02)
			//by-pass pooling
			write_channel_intel(bypass_ch_L3, conv_ch_in);
		else // to pooling kernel

			write_channel_intel(conv_ch_L3, conv_ch_in);
			...
			)
{
	...
	for(unsigned int k=0; k<input_num; k++){
		...
		conv_ch_out = read_channel_intel(conv_ch_L3);
		...
		if(row_pool_cnt==(pool_size-1)){
			...
			if(col_pool_cnt==(pool_size-1)){
				...
				write_channel_intel(pool_ch_L3, pool_final);
				...
				)
{
	...
	if(local_z==0){
		if((bypass&0x01)==0x01)
			output = read_channel_intel(bypass_ch_L3);

		else
			output = read_channel_intel(pool_ch_L3);
    ...
    if (global_x >= 0 && global_x <= 3) {
        if (global_y >= 0 && global_y <= 3) {
            if (global_z >= 0 && global_z <= 3) {
                printf ("[lrn L3] at x=%d,y=%d,z=%d = [", global_x, global_y, global_z);
                for (unsigned char i = 0; i < VEC_SIZE; i++) {
...
/*
                if (dim1 >= 0 && dim1 <= 3) {
                    if (dim2 >= 0 && dim2 <= 3) {
                        if (dim3 >= 0 && dim3 <= 3) {
                            printf ("[ser L3] at x=%d,y=%d,z=%d = [", dim1, dim2, dim3);
...
*/
				write_channel_intel (sdeser_ch_L3, buf);
			}
		}
	}
