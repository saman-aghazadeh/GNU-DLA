...
channel channel_vec    data_ch_1   __attribute__((depth(0)));
channel channel_vec    weight_ch_1 __attribute__((depth(0)));
channel channel_scal   bias_ch_1   __attribute__((depth(8)));
channel channel_vec    data_ch_2   __attribute__((depth(0)));
channel channel_vec    weight_ch_2 __attribute__((depth(0)));
channel channel_scal   bias_ch_2   __attribute__((depth(8)));
channel channel_scal   conv_ch_1   __attribute__((depth(CHN_DEPTH)));
channel channel_scal   conv_ch_2   __attribute__((depth(CHN_DEPTH)));
channel channel_scal   pool_ch     __attribute__((depth(CHN_DEPTH)));
channel channel_scal   memwr_ch    __attribute__((depth(CHN_DEPTH)));
channel channel_vec    arb_ch      __attribute__((depth(0)));
channel channel_scal   bypass_ch_1 __attribute__((depth(CHN_DEPTH)));
channel channel_scal   bypass_ch_2 __attribute__((depth(CHN_DEPTH)));
	...
	for(unsigned int out_idx_xyz=0; out_idx_xyz<(weight_dim4_div_lane*group_num_y*group_num_x); out_idx_xyz++){
		...
		for(unsigned int win_itm_xyz = 0; win_itm_xyz < item_loop_bound; win_itm_xyz++) {
			...
			if(gp_num_x*CONV_GP_SIZE_X+gp_item_idx_x<conv_x){
				...
				write_channel_intel(arb_ch, data_ch_vec);
			}
...
void memReadDataArb(
		...
		uchar  data_ch_toggle = 0;
		channel_vec   data_ch_vec;
		for (int j = 0; j < output_num; j++) {
			for (int i = 0; i < conv_loop_cnt; i++) {
				data_ch_vec = read_channel_intel(arb_ch);
				if (data_ch_toggle == 0) {
					write_channel_intel(data_ch_1, data_ch_vec);
				} else {
					write_channel_intel(data_ch_2, data_ch_vec);
				}
			}
			//printf ("[FPGA] memReadDataArb for %d\n", j);
			data_ch_toggle = (~data_ch_toggle) & 0x01;
		}			
}
__kernel
__attribute__((task))
...
{	
	...
	for (ushort i = 0; i < weight_dim4_div_lane; i++) {
		...
		for (ushort j = 0; j < conv_xy; j++) {
			...
			if (data_ch_toggle == 0) {
				write_channel_intel(bias_ch_1, bias_ch_in);
			} else {
				write_channel_intel(bias_ch_2, bias_ch_in);
			}
			data_ch_toggle = (~data_ch_toggle) & 0x01;
		}
	}
	...
	for (ushort i = 0; i < weight_dim4_div_lane; i++) {
		...
		for (uint j = 0; j < conv_xxy; j++) {
			...
			for (uint p = 0; p < weight_dim1x2x3/VEC_SIZE; p++) {
				...
				if (data_ch_toggle == 0) {
					write_channel_intel(weight_ch_1, weight_ch_vec);
				} else {
					write_channel_intel(weight_ch_2, weight_ch_vec);
				}
			}
			data_ch_toggle = (~data_ch_toggle) & 0x01;
			...
			)
{
	...
	for(unsigned int k=0; k<output_num; k++){
		//printf ("[FPGA] Waiting for bias_ch_1\n");
		bias_ch_out = read_channel_intel(bias_ch_1);
		//if (k % 128 == 0)
		//printf ("[FPGA] Got something on coreConv1 for output %d\n", k);
		#pragma unroll
		for(unsigned char ll=0; ll<LANE_NUM; ll++){
		...
		for(int j=0; j<conv_loop_cnt; j++){
			//printf ("[FPGA] waiting for data_ch_1\n");
			mac_data = read_channel_intel(data_ch_1);
			//printf ("[FPGA] waiting for weight_ch_1\n");
			mac_weight = read_channel_intel(weight_ch_1);
			//printf ("[FPGA] We are good 1!\n");
		...
		if((contol&0x02)==0x02)
			//by-pass pooling
			write_channel_intel(bypass_ch_1, conv_ch_in);
		else // to pooling kernel
			write_channel_intel(conv_ch_1, conv_ch_in);
			...
			)
{
	...
	// each iteration generates one output
	for(unsigned int k=0; k<output_num; k++){
		//printf ("[FPGA] Waiting for bias_ch_2\n");
		bias_ch_out = read_channel_intel(bias_ch_2);
		//if (k % 128 == 0)
		//printf ("[FPGA] Got something on coreConv2 for output %d\n", k);
		#pragma unroll
		for(unsigned char ll=0; ll<LANE_NUM; ll++){
		...
		for(int j=0; j<conv_loop_cnt; j++){
			//printf ("[FPGA] Waiting for data_ch_2\n");
			mac_data = read_channel_intel(data_ch_2);
			//printf ("[FPGA] Waiting for weight_ch_2\n");
			mac_weight = read_channel_intel(weight_ch_2);
			//printf ("[FPGA] We are good 2!\n");
		...
		if((contol&0x02)==0x02)
			//by-pass pooling
			write_channel_intel(bypass_ch_2, conv_ch_in);
		else // to pooling kernel
			write_channel_intel(conv_ch_2, conv_ch_in);
			//printf("Write channel item-%d is written in channel %d...\n", k, ll);
			...
			)
{
	...
	for(unsigned int k=0; k<input_num; k++){
		...
		if (data_ch_toggle == 0) {
			conv_ch_out = read_channel_intel(conv_ch_1);
			data_ch_toggle = 1;
		} else {
			conv_ch_out = read_channel_intel(conv_ch_2);
			data_ch_toggle = 0;
		}
...
// Retrieve bypass elements from the convs
__kernel
__attribute__((task))
void bypassIntercept(
				uint out_dim1x2x3xbatch,
				uchar bypass
			) 
{
	uchar data_ch_toggle = 0;
	for (unsigned int ll = 0; ll < out_dim1x2x3xbatch; ll++) {
		channel_scal input;
		data_ch_toggle = ll & 0x01;

		if (data_ch_toggle == 0) {
			input = read_channel_intel(bypass_ch_1);
		} else {
			input = read_channel_intel(bypass_ch_2);
		}
		//printf ("[FPGA] Bypass receive data from bypass channel of %d in ll of %d\n", data_ch_toggle, ll);
		write_channel_intel(memwr_ch, input);
	}
				...
				)
{
	...
	if(local_z==0){
		if((bypass&0x01)==0x01) {
			output = read_channel_intel(memwr_ch);
		} else {
