...
typedef struct mm_thread_wrap {
	mkl_config config[FC_NUM];
};
...
void* mm_cpu_calculation(void* arg);
...
int main(int argc, char** argv)
{
	...
	printf ("[INFO] Program created successfully!\n");
	...
	struct mm_thread_wrap *cpu_mm_configs = (struct mm_thread_wrap*) malloc (sizeof(struct mm_thread_wrap));
	...
	for (int i = 0; i < FC_NUM; i++) {
		// The numbers being pushed to the functions are based on how the 
		// dimensions are given as the input!
		init_mkl_config(&((cpu_mm_configs->config)[i]),
			layer_config[LAYER_NUM+i][weight_m],
			layer_config[LAYER_NUM+i][data_w]*layer_config[LAYER_NUM+i][data_n]);
	}


	// Maybe warming up the cache for the matrix multiplications
	for (int i = 0; i < 100; i++) {
		mm_cpu_calculation((void *) cpu_mm_configs);
	}	
	...
	for (int iter = 0; iter < 5; iter++) {
		...
		printf ("[INFO] Before the join!\n");
		if (iter != 0)
			pthread_join(mm_cpu_thread_id, NULL);
		printf ("[INFO] After the join!\n");
		pthread_create (&mm_cpu_thread_id, NULL, mm_cpu_calculation, (void *) cpu_mm_configs);
...
void* mm_cpu_calculation(void* input_arg) {
	struct mm_thread_wrap* config = (struct mm_thread_wrap *) input_arg;

	double s_initial, s_elapsed;

	for (int i = 0; i < FC_NUM; i++) {
		// printf ("[INFO] Start calculating for the fully connected layer %d\n", i);
		// s_initial = dsecnd();
		run_mkl(&((config->config)[i]));
		// s_elapsed = (dsecnd() - s_initial);
		// printf ("[INFO] Layer %d took %.5f milliseconds to finish\n\n", i, (s_elapsed * 1000));
	}	
}
