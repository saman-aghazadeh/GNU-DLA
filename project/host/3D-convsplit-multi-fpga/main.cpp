...
#include "../../device/3D-convsplit-multi-fpga/hw_param.cl"
...
scoped_array<scoped_array<int>> num_sub_layers;
...
int main(int argc, char** argv)
{
	...
	num_sub_layers.reset(num_devices);
	for (int i = 0; i < num_devices; i++) {
		...
		num_sub_layers[i].reset(num_layers_involved);
	...
	for (int i = 0; i < num_devices; i++) {
		...
		for (int layer = 0; layer < layers_per_device[i]+1; layer++) {
			...
			if(WEIGHT_BUF_SIZE < layer_config[assigned_layers[i][0]+layer-1][weight_h] * (layer_config[assigned_layers[i][0]+layer-1][weight_n] / VEC_SIZE) * layer_config[assigned_layers[i][0]+layer-1][weight_t]) {
				...
				int max_sub_layer_channels = (VEC_SIZE * WEIGHT_BUF_SIZE) / (layer_config[assigned_layers[i][0]+layer-1][weight_h] * layer_config[assigned_layers[i][0]+layer-1][weight_t]);
				fpga_config[layer].num_sub_layers = (layer_config[assigned_layers[i][0]+layer-1][weight_n] - 1) / max_sub_layer_channels + 1;
			}
			else
				fpga_config[layer].num_sub_layers = 1;

			num_sub_layers[i][layer] = fpga_config[layer].num_sub_layers;
			printf ("Num sub layers is %d\n", fpga_config[layer].num_sub_layers);
...
void* device_runner (void* args) {
	...
	for (int iter = 0; iter < 4; iter++) {
		...
		argi = 0;
		char config_size = 0;
		// start of config size computation
		for (int layer = 0; layer < LAYER_NUM; layer++) 
			config_size += num_sub_layers[i][layer];
		// end of config size computation
