...
typedef struct {
	...
	int memwr_dst;
	int num_bricks;
	int conv_group_size_w, conv_group_size_h, conv_group_size_t;
} fpga_configuration;
...
int main(int argc, char** argv)
{
	...
	char *kernel_file_name="conv.aocx";//argv[1];
	...
	for (int layer = 0; layer < LAYER_NUM; layer++) {
        ...
        int conv_group_size_w_arr[10]={6,6,6,6,6,6,6,6,1,1};
        int conv_group_size_h_arr[10]={2,2,2,2,2,2,2,2,1,1};
        int conv_group_size_t_arr[10]={2,2,2,2,2,2,2,2,1,1};
        config[layer].conv_group_size_w = conv_group_size_w_arr[layer];//layer_config[layer][conv_group_size_w];
        config[layer].conv_group_size_h = conv_group_size_h_arr[layer];//layer_config[layer][conv_group_size_h];
        config[layer].conv_group_size_t = conv_group_size_t_arr[layer];//layer_config[layer][conv_group_size_t];
