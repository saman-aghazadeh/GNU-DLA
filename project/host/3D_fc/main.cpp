...
#define FC_BOTTOM0_BUF_SIZE	25088
#define FC_BOTTOM1_BUF_SIZE	4096
#define FC_WEIGHT_SIZE 123731968
#define FC_WEIGHT_OFFSET 229896
...
#define FC_BOTTOM1_BUF_SIZE	4096
#define FC_WEIGHT_SIZE 52326400
#define FC_WEIGHT_OFFSET 1152576
const char *weight_file_path = "./data/data_vgg16/weights.dat";
const char *input_file_path = "./data/data_vgg16/image.dat";
#endif
...
const char *knl_name_fcMemRd = "fc_memRead";
const char *knl_name_controller = "controller";
const char *knl_name_memWrite = "memWrite";
const char *knl_name_fcMemWrite = "fc_memWrite";
...
int main(int argc, char** argv)
{
	...
	que_fcMemRd = clCreateCommandQueue(context, device[0], CL_QUEUE_PROFILING_ENABLE, &status);
	checkError(status, "Failed to create command queue for memRdWeight");
	...
	que_fcMemWrite = clCreateCommandQueue(context, device[0], CL_QUEUE_PROFILING_ENABLE, &status);
	checkError(status, "Failed to create command queue for memWrite");
	...
	knl_fcMemRd = clCreateKernel(program, knl_name_fcMemRd, &status);
	checkError(status, "Failed to create fcMemRd kernel");
	...
	knl_fcMemWrite = clCreateKernel(program, knl_name_fcMemWrite, &status);
	checkError(status, "Failed to create fcMemWrite kernel");
	...
	printf ("[INFO] Creating the fc weight buffer\n");
	fc_weights_buf = clCreateBuffer(context, CL_MEM_READ_ONLY, FC_WEIGHT_SIZE * sizeof(DTYPE), NULL, &status);
	checkError(status, "Failed to create buffer for weights in fc layer");
	...
	printf ("[INFO] Enqueueing the fc weight buffer to weight queue\n");
	status = clEnqueueWriteBuffer(que_fcMemRd, fc_weights_buf, CL_TRUE, 0, FC_WEIGHT_SIZE * sizeof(DTYPE), weights + FC_WEIGHT_OFFSET, 0, NULL, NULL);
	checkError(status, "Failed to transfer weight");
	...
	// First fc buffer
	printf ("[INFO] Creating the fc bottom0 buffer\n");
	fc_bottom0_buf = clCreateBuffer(context, CL_MEM_READ_WRITE, FC_BOTTOM0_BUF_SIZE * sizeof(DTYPE), NULL, &status);
	checkError(status, "Failed to create buffer for fc data in layer");

	// Second fc buffer
	printf ("[INFO] Creating the fc bottom1 buffer\n");
	fc_bottom1_buf = clCreateBuffer(context, CL_MEM_READ_WRITE, FC_BOTTOM1_BUF_SIZE * sizeof(DTYPE), NULL, &status);
	checkError(status, "Failed to create buffer for fc output");
	...
	for (int layer = 0; layer < LAYER_NUM; layer++) {
		...
		printf("[2D] num_bricks_w: %d num_bricks_h: %d num_bricks_t: %d\n", num_bricks_w, num_bricks_h, num_bricks_t);
		printf ("[INFO] w_vec: %d\n", w_vec);
	...
	for (int iter = 0; iter < 4; iter++) {
		...
		argi = 0;
		char local_precision_config_0 = 7;
		char local_precision_config_1 = 0;
		char local_precision_config_2 = 2;
		...
		// status = clSetKernelArg(knl_controller, argi++, sizeof(cl_char), &local_precision_config_0);
		// checkError(status, "Failed to set argument %d of kernel controller", argi-1);

		// status = clSetKernelArg(knl_controller, argi++, sizeof(cl_char), &local_precision_config_1);
		// checkError(status, "Failed to set argument %d of kernel controller", argi-1);
		...
		status = clSetKernelArg(knl_fcMemRd, argi++, sizeof(cl_mem), &fc_bottom0_buf);
		checkError(status, "Failed to set argument %d of kernel memory fc read", argi-1);

		status = clSetKernelArg(knl_fcMemRd, argi++, sizeof(cl_mem), &fc_bottom1_buf);
		checkError(status, "Failed to set argument %d of kernel memory fc read", argi-1);

		status = clSetKernelArg(knl_fcMemRd, argi++, sizeof(cl_mem), &fc_weights_buf);
		checkError(status, "Failed to set argument %d of kernel memory fc read", argi-1);

		status = clSetKernelArg(knl_fcMemRd, argi++, sizeof(cl_mem), &bias_buf);
		checkError(status, "Failed to set argument %d of kernel memory fc read", argi-1);

		argi = 0;

		printf ("[INFO] Setting kernel arguments for the fcMemWrite\n");
		status = clSetKernelArg(knl_fcMemWrite, argi++, sizeof(cl_char), &config_size);
