...
int main(int argc, char** argv)
{
	...
	for (int layer = 0; layer < LAYER_NUM; layer++) {
		...
		// Layer type of 0 means convolution
		// Layer type of 1 means matrix multiplication
		if (layer_config[layer][layer_type] == 0) {
			config[layer].data_w = layer_config[layer][data_w];
			config[layer].data_h = layer_config[layer][data_h];	
		} else {
			config[layer].data_w = W_VEC;
			config[layer].data_h = layer_config[layer][data_h];
		}
		if (layer_config[layer][layer_type] == 0) {
		...
		if (layer_config[layer][layer_type] == 0) {
			...
			config[layer].weight_n = layer_config[layer][weight_n] / W_VEC + 1;
			config[layer].weight_m = layer_config[layer][weight_m];
		}
