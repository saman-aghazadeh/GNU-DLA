...
int main(int argc, char** argv)
{
	...
	//context[2] = clCreateContext(NULL, 1, &(device[device_mapping[2]]), NULL, NULL, &status);	 checkError(status, "Failed to create context");
	...
	for (int i = 0; i < num_devices; i++) {
		...
		printf ("[INFO] Layers per device is %d\n", layers_per_device[i]);
...
int prepare()
{
	...
	for(unsigned ll=0; ll<LAYER_NUM; ll++){
		...
		printf ("[INFO] weight_m of original layer config is %d\n", layer_config[ll][weight_m]);
