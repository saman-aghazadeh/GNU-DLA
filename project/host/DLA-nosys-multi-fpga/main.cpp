...
int main(int argc, char** argv)
{
	...
	for (int i = 0; i < num_devices; i++) {
		...
		for (int j = 0; j < layers_per_device[j]; j++) {
	...
	for (int i = 0; i < num_devices; i++) {
		...
		printf ("[INFO] Setting the configuratioNS PER layer for the " ANSI_COLOR_RED "DEVICE %d " );
		for (int layer = 0; layer < layers_per_device[i]+1; layer++) {
			...
			printf ("[INFO] w_vec: %d\n", w_vec);
			printf ("[INFO] data_w: %d\n", layer_config[assigned_layers[i][0]+layer][data_w]);
			printf ("[INFO] data_h: %d\n", layer_config[assigned_layers[i][0]+layer][data_h]);
			printf ("[INFO] conv_padding: %d\n", layer_config[assigned_layers[i][0]+layer][conv_padding]);
			printf ("[INFO] weight_w: %d\n", layer_config[assigned_layers[i][0]+layer][weight_w]);	
			printf ("[INFO] Some #1: %d\n", layer_config[layer][data_h]+2*layer_config[assigned_layers[i][0]+layer][conv_padding]-layer_config[assigned_layers[i][0]+layer][weight_h]+1);
			printf ("[INFO] first part: %d\n", (layer_config[assigned_layers[i][0]+layer][data_w]+2*layer_config[assigned_layers[i][0]+layer][conv_padding]-layer_config[assigned_layers[i][0]+layer][weight_w]));
			printf ("[INFO] second part: %d\n", (w_vec-layer_config[assigned_layers[i][0]+layer][weight_w]+1));
			printf ("[INFO] Some #2: %d\n", ((layer_config[assigned_layers[i][0]+layer][data_w]+2*layer_config[assigned_layers[i][0]+layer][conv_padding]-layer_config[assigned_layers[i][0]+layer][weight_w])/(w_vec-layer_config[assigned_layers[i][0]+layer][weight_w]+1)) + 1);
